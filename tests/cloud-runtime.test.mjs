...
function fakeCloudClient(options = {}) {
  ...
  return {
    ...
    signInWithGoogle() {
      ...
      return Promise.resolve(options.popupResult ?? { status: "pending" });
    },
    signOut() {
    ...
    async saveSharedArtifact(input) {
      ...
      return structuredClone(options.sharedSaveResult ?? {
        status: "verified",
...
function harness({ initialState = stateFixture(), cloud = fakeCloudClient() } = {}) {
...
test("demo actions perform no storage or cloud operation and always expose an exit route", () => {
  ...
  const beforeCalls = app.cloud.calls.length;
  const actions = app.controller.getSetupActions();
  ...
  assert.equal(app.cloud.calls.length, beforeCalls);
