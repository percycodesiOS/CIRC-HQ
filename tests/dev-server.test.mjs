...
async function withServer(context) {
  ...
  await Promise.all([
    ...
    writeFixture(root, "fid.html", "GENERIC_FID"),
    writeFixture(root, "fid.css", "GENERIC_FID_CSS"),
    writeFixture(root, "walkthrough.html", "GENERIC_WALKTHROUGH"),
...
test("the real dev server serves only the explicit public runtime allowlist", async (context) => {
  ...
  for (const [pathname, expectedBody] of [
    ...
    ["/app.css", "GENERIC_CSS"],
    ["/fid.html", "GENERIC_FID"],
    ["/fid.css", "GENERIC_FID_CSS"],
    ["/walkthrough.html", "GENERIC_WALKTHROUGH"],
