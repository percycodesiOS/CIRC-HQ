...
## 2026-09-10 Student production work area

Extended Kenny's authorized CIRC release with a student work area reachable from both the announcement studio and private crew sign-up. Reporter, weather and short iMovie video desks each present one manual step at a time through preparation, creation and teacher review. The video assignment uses one camera, three shots and a 15-45-second result; it preserves the Wednesday September 9 and Thursday September 10 work window.

Official Apple creation, trim and export instructions and NWS Cranberry Township forecast/hourly plus Pittsburgh radar links were checked. The runtime verifier admits only the six exact reviewed destinations. These are explicit links opened by the user, with no geolocation request, external embed or background fetch.

The work area takes no teacher plan, crew list or private script. It hides the teacher header and schedule live region, stores only its selected desk/step in memory, and returns to the announcement studio through an explicit teacher exit. It provides directions; students write on paper and edit in iMovie. No media is collected or uploaded. Existing Day 5 build, CIRC mark, crew sign-up and announcement approval gates are preserved.

Focused tests cover every desk and step, invalid selection bounds, next-step callbacks, missing private identities and input/media controls, exact official-link admission, and the actual private-signup to student-view to teacher-studio flow without extra storage writes. Final validation and deployment evidence are saved in the current task's student-work-area release report.

## 2026-09-10 Morning Ehrman Show design refinement

Applied the user's visual feedback to the student studio and teacher crew sign-up within the authorized release. The student page now has the correct Morning Ehrman Show identity, an original generic studio photograph, a balanced editorial layout, three prominent work desks, a visual three-shot brief, a reporter notebook and immediate official weather tools. Each active task has three short actions; full reviewed directions are still accessible. The student and operator text uses teacher instead of a first name. Teacher preparation and sign-up clearly say TEACHER BACKSTAGE / KEEP OFF PROJECTOR.

Replaced the ambiguous circuit glyph in both identical HTML headers and the favicon with the supplied, unchanged SV interlock PNG. Header and student masthead preserve its original aspect ratio. Public asset manifests admit only the two reviewed new images with exact binary hash locks; PNG is served with its correct MIME type. Student desks still receive no private data, make no storage writes and collect no media. Day 5 build timing, local roster validation, scripts and approval gates remain intact.

Desktop screenshots and 390-pixel browser checks were inspected. A root quality review found no layout/privacy blocker and identified an initial logo rendering ratio issue, corrected before release. This is internal quality review, not a claim that the user approved the design or that an unavailable Tank page was matched. Final test and live deployment evidence are in the current readiness report.
